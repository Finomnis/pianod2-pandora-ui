// TODO
// - Wait for authentication changed / socket initialized
...
function* authenticate(credentials: { username: string, password: string }) {
    try {
        yield call(pianod2_client.execute_command.bind(pianod2_client), "authenticate", credentials);
    } catch (e) {
...
export function* authenticationSaga() {
    while (true) {
        type RaceResult = {
            connection_established: (ReturnType<typeof connectionEstablished>) | undefined,
            credentials_changed: (ReturnType<typeof credentialsChanged>) | undefined,
        };
        ...
        if (connection_established) {
            const credentials: { username: string, password: string } = yield select(getCredentials);
